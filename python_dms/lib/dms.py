...
class DMS:
    ...
    def Addblur(self):
        if self.blur_type == "Gaussian":
            ...
            kernel = matlab_style_gauss2D(
                (self.blur_size, self.blur_size), self.blur_std
            )
    ...
    def R_prox(self, e, tau):
        if self.norm_type == "l0":
            ...
            C1 = np.eye(self.rowscols * 2) * (
                1 + (2 * self.lam / (4 * self.eps * self.lam / tau))
            )
            C2 = np.ones((self.rowscols * 2, self.rowscols * 2)) * (
                2 * self.lam * self.eps * self.lam / tau
            )
            mat = csc_matrix(identity(self.rowscols * 2) * C1) + csc_matrix(
                C2 * self.optD1.T.dot(self.optD1)
            )
    ...
    def optD(self, x):
        ...
        if self.canal == 3:
            ...
            y[:, :, 0, :] = (
                np.concatenate(
                    (x[:, 1:, :] - x[:, 0:-1, :], np.zeros((self.rows, 1, self.canal))),
                    axis=1,
                )
                / 2.0
            )
            y[:, :, 1, :] = (
                np.concatenate(
                    (x[1:, :, :] - x[0:-1, :, :], np.zeros((1, self.cols, self.canal))),
                    axis=0,
                )
                / 2.0
            )
            ...
            y[:, :, 0] = (
                np.concatenate(
                    (x[:, 1:] - x[:, 0:-1], np.zeros((self.rows, 1))), axis=1
                )
                / 2.0
            )
            y[:, :, 1] = (
                np.concatenate(
                    (x[1:, :] - x[0:-1, :], np.zeros((1, self.cols))), axis=0
                )
                / 2.0
            )
            # dh = np.array([[-1,1]])/2
    ...
    def Dadjoint(self, x):
        if self.canal == 3:
            y1 = (
                np.concatenate(
                    (
                        x[:, 0, 0, :].reshape(self.rows, 1, 3),
                        x[:, 1:-1, 0, :] - x[:, 0:-2, 0, :],
                        -x[:, -1, 0, :].reshape(self.rows, 1, 3),
                    ),
                    axis=1,
                )
                / 2
            ...
            y2 = (
                np.concatenate(
                    (
                        x[0, :, 1, :].reshape(1, self.cols, 3),
                        x[1:-1, :, 1, :] - x[:-2, :, 1, :],
                        -x[-1, :, 1, :].reshape(1, self.cols, 3),
                    ),
                    axis=0,
                )
                / 2
            ...
            y1 = (
                np.concatenate(
                    (
                        x[:, 0, 0].reshape(self.rows, 1),
                        x[:, 1:-1, 0] - x[:, 0:-2, 0],
                        -x[:, -1, 0].reshape(self.rows, 1),
                    ),
                    axis=1,
                )
                / 2.0
            ...
            y2 = (
                np.concatenate(
                    (
                        x[0, :, 1].reshape(1, self.cols),
                        x[1:-1, :, 1] - x[:-2, :, 1],
                        -x[-1, :, 1].reshape(1, self.cols),
                    ),
                    axis=0,
                )
                / 2.0
    ...
    def loop_PALM_eps_descent(self):
        ...
        while self.eps >= self.eps_AT_min:  # and time.time()- time_start_PALM<60:
            ...
            ) and iteration < self.MaximumIteration:  # //np.ceil(np.log2(self.eps/0.02)): #and np.abs((temp_err-err)/err)>= self.stop_criterion):and time.time()- time_start_local<self.time_limit
            ...
            while (
                ...
                if self.norm_type != "AT" and self.norm_type != "AT-fourier":
                    ...
                    over = (
                        ...
                        + self.dk_SLPAM / 2.0 * self.en_SLPAM
                    )
                    ...
                    next_en_SLPAM = (
                        self.en_SLPAM
                        + 2 * self.beta / self.dk_SLPAM * self.optD(next_un_SLPAM) ** 2
                    )
                    ...
                    # print(temp-temp1)
                    next_en_SLPAM[:, :, 0] = temp[: self.rowscols].reshape(
                        self.rows, self.cols
                    )
                    next_en_SLPAM[:, :, 1] = temp[self.rowscols :].reshape(
                        self.rows, self.cols
                    )
    ...
    def loop_SL_PAM(self):
        ...
        while (err > self.stop_criterion) and (
        ...
        ):  # and time.time()-time_start_SLPAM<self.time_limit
            ...
            if self.norm_type == "l1" or self.norm_type == "l1q":
                ...
                self.b = 0.5 * self.ck_SLPAM[it] - np.sum(
                    (1 - self.en_SLPAM) ** 2
                ) ** 2 / (2 * mu)
                self.mx = (
                    0.5 * self.ck_SLPAM[it]
                    + 0.5 * rho_l
                    - Lips_grad_f**2 * mu / 2
                    - nu * Lips_f**2 / 2
                )
    ...
    def loop_iPALM(self):
        ...
        while (err > self.stop_criterion) and (it < self.MaximumIteration):
            ...
            self.un_PALM = self.L_prox(
                y1 - (self.beta / tau_1) * self.S_du(z1, self.en_PALM),
                1 / tau_1,
                self.image_degraded,
            )
            ...
            self.en_PALM = self.R_prox(
                y2 - (self.beta / tau_2) * self.S_de(self.un_PALM, z2), self.lam / tau_2
            )
        ...
        if show == True:
            if self.method != "compare":
                ...
                if self.method == "SLPAM":
                    ...
                    if self.draw == "points":
                        ...
                        for i in range(0, len(xv)):
                            f6.plot(
                                [xv[i] - 0.5, xv[i] + 0.5],
                                [self.cols - (yv[i] + 0.5), self.cols - (yv[i] + 0.5)],
                                "r-",
                            )
                        for i in range(0, len(xh)):
                            f6.plot(
                                [xh[i] + 0.5, xh[i] + 0.5],
                                [self.cols - (yh[i] - 0.5), self.cols - (yh[i] + 0.5)],
                                "r-",
                            )
                    ...
                    f5 = f.add_subplot(335)
                    f5.plot(
                        self.error_curve_PALM[1 : self.it_PALM], label="Error curve"
                    )
                    f5.legend()
                ...
                if self.draw == "points":
                    ...
                    f10.plot(
                        self.error_image_SLPAM[: self.it_SLPAM], label="SL-PALM error"
                    )
                    f10.legend()
                ...
                plt.savefig(
                    "../out/out_all/[{}][beta={}][lambda={}][dk={}][Iteration={}][norm={}][{}-{}-{}][{}-{}][{}].jpg".format(
                        self.method,
                        self.beta,
                        self.lam,
                        self.dk_SLPAM,
                        self.MaximumIteration,
                        self.norm_type,
                        self.blur_type,
                        self.blur_size,
                        self.blur_std,
    ...
    def process(self):
        if self.noised_image_input is None:
            ...
            self.AddNoise()
            im = Image.fromarray(
                (self.normalization(self.image_degraded) * 255).astype(np.uint8)
            )

            if self.noise_type == "Gaussian" and self.blur_type == "Gaussian":
                im.save(
                    "../noisy_image/[{}-{}-{}][{}-{}][{}].png".format(
                        self.blur_type,
                        self.blur_size,
                        self.blur_std,
                        self.noise_type,
        ...
        if self.method == "SLPAM":
            ...
            (
                contour_iPALM,
                restored_image_iPALM,
                degraded_image_iPALM,
                energy_iPALM,
                error_iPALM,
                dk_iPALM,
                ck_iPALM,
