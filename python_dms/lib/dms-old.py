...
class DMS():
    def __init__(self,img,save_name,blur_type='Gaussian',blur_size=7,blur_std=1.1,
        ...
        gamma_1= 1e-2,gamma_2 = 1e-2,time_limit=3600):
    ...
    def Addblur(self):
        if self.blur_type == 'Gaussian':
            ...
            self.image_degraded = self.normalization(self.image_degraded)
    ...
    def Aadjoint_process(self, im):
        ...
        self.Aadjoint = self.normalization(self.Aadjoint)
    ...
    def R_prox(self,e,tau):
        if self.norm_type== 'l0':
            ...
            # print(F)
            # e[:,:,0] = self.normalization(e[:,:,0])
            # e[:,:,1] = self.normalization(e[:,:,1])
            return e
        elif self.norm_type =='l1l2':
        ...
        if(self.canal==3):
            ...
            if self.blur_type == 'none':
                if self.type_u0 == 'Z':
                    ...
                    self.un_SLPAM = self.normalization(self.un_SLPAM)
        ...
        if (self.canal == 3):
            ...
            if self.blur_type == 'none':
                if self.type_u0 == 'Z':
                    ...
                    self.un_PALM = self.normalization(self.un_PALM)
            elif self.blur_type == 'Gaussian':
...
#                 rhok_e= np.sqrt(np.sum(xn1**2))
            ...
            return 1.01 * self.beta * np.sqrt(rhok_u)+1e-8,1.01*self.beta*np.sqrt(rhok_e)+1e-8
    ...
    def loop_PALM_eps_descent(self):
        ...
        while self.eps>=self.eps_AT_min:# and time.time()- time_start_PALM<60:
            ...
            err=1
            while (err >= self.stop_criterion) and time.time()- time_start_local<self.time_limit:# and iteration <self.MaximumIteration//np.ceil(np.log2(self.eps/0.02)): #and np.abs((temp_err-err)/err)>= self.stop_criterion):
            ...
            # plt.axis('off')
            # plt.imshow(self.normalization(self.un_PALM),'gray',vmin=0,vmax=1)
    ...
    def loop_SL_PAM_eps_descent(self):
        ...
        while self.eps>=self.eps_AT_min:# and time.time()- time_start_SLPAM<60:
            ...
            err=1
            k=1
            while (err >= self.stop_criterion and time.time()- time_start_local<(k+4)*60):
                ...
                # self.en_SLPAM[:,:,0] = self.normalization( self.en_SLPAM[:,:,0])
                # self.en_SLPAM[:,:,1] = self.normalization( self.en_SLPAM[:,:,1])
            ...
            f=plt.figure(1)
            plt.axis('off')
            plt.imshow(self.normalization(self.un_SLPAM),'gray',vmin=0,vmax=1)
    ...
    def loop_SL_PAM(self):
        ...
        # Main loop
        time_start_SLPAM = time.time()#
        while (err > self.stop_criterion) and time.time()-time_start_SLPAM<self.time_limit and (it < self.MaximumIteration): 
    ...
    def loop_iSL_PAM(self):
        ...
        while (err > self.stop_criterion) and (it < self.MaximumIteration): 
            ...
            if(self.norm_type!='AT' and  self.norm_type != 'AT-fourier'):
                ...
                # print(np.max(self.en_SLPAM))
            un_SLPAM_previous = self.un_SLPAM
            en_SLPAM_previous = self.en_SLPAM

            # print(self.L_term(self.un_SLPAM,self.image_degraded) ,self.beta*self.S_term(self.un_SLPAM,self.en_SLPAM) , self.lam*self.R_term(self.en_SLPAM))
    ...
    def loop_PALM(self):
        ...
        # plt.show()
        time_start_PALM = time.time()
        while (err > self.stop_criterion) and  time.time()-time_start_PALM<self.time_limit:#and (it < self.MaximumIteration):
            # ..............
        ...
        # self.en_PALM[:,:,0] = self.normalization(self.en_PALM[:,:,0])
        # self.en_PALM[:,:,1] = self.normalization(self.en_PALM[:,:,1])
    ...
    def process(self):
        if (self.noised_image_input is None):
            ...
            if np.max(self.noised_image_input)>1:
                self.image_degraded = self.normalization(self.noised_image_input / 255.)
            else:
                self.image_degraded = self.normalization(self.noised_image_input)
            # self.image_degraded = self.normalization(self.noised_image_input)
            if self.blur_type == 'Gaussian':
                # fs = (self.blur_size - 1) / 2.
                # x = np.arange(-fs, fs + 1)
