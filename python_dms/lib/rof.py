...
def PA_PDtv(I, chi, Niter=300):
    ...
    while k < Niter - 1 and eps_crit >= 1e-4:
        ...
        dual_crit[k + 1] = (
            0.5 * np.sum(p**2)
            - chi * np.sum(p * I)
            + np.sum(np.sqrt(p1**2 + p2**2))
        )
        # table_energy[k] = 0.5*np.sum((DivHor(v_n))**2)+lamb*np.sum(np.abs(GradientHor(y+DivHor(v_n))))

        # Primal crit
        [ph, pv] = opL(x_n)
        primal_crit[k + 1] = 0.5 * np.sum((I - x_n) ** 2) + chi * np.sum(
            np.sqrt(np.sum(ph**2 + pv**2))
        )
...
def optD_normalized(x):
    ...
    # # print(temp.shape)
    y[:, :, 0] = (
        np.concatenate((x[:, 1:] - x[:, 0:-1], np.zeros((rows, 1))), axis=1) / 2.0
    )
    y[:, :, 1] = (
        np.concatenate((x[1:, :] - x[0:-1, :], np.zeros((1, cols))), axis=0) / 2.0
    )
    y = np.abs(y)
    y = y > 0
    return y
